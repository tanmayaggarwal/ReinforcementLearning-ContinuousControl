...
In this project, I have used the Deep Deterministic Policy Gradient (DDPG) algorithm to train the double jointed arm to reach the target location.

DDPG is a deep reinforcement learning algorithm which concurrently learns:
...
This algorithm uses off-policy data and the Bellman equation to learn the Q-function (i.e., the data that is used is asynchronous from the actual policy being used in any given time step), and uses the Q-function to learn the policy.

DDPG is specifically adapted for environments with continuous action spaces (versus the Deep Q-learning algorithm which is better for discrete action spaces) as using a normal optimization algorithm (e.g., argmax) would be computationally expensive and functionally unacceptable. In DDPG, because the action space is continuous, the Q-function is presumed to be differentiable with respect to the action allowing us to set up an efficient, gradient-based learning rule for updating the policy.

Additional details of the DDPG algorithm can be found here: https://spinningup.openai.com/en/latest/algorithms/ddpg.html
