...
class Actor(nn.Module):
    ...
    def forward(self, state):
        # Build an actor (policy) network that maps state -> action values
...
class Critic(nn.Module):
    ...
    def forward(self, state, action):
        ...
        if torch.cuda.is_available():
            action = action.type(torch.cuda.FloatTensor).squeeze()
        else:
            action = action.type(torch.FloatTensor).squeeze()
